...
def funcao_orse(ano: str, mes: str, tipo: str):
    horario_inicio_scraping = datetime.now().time()

    # Fazer possível conversão de formato de variáveis para ano e mês, aparentemente dia é uma variável fixa.
    print("VALOR DE ANO: ", ano)
    print("VALOR DE MÊS: ", mes)
    print("VALOR DE TIPO: ", tipo)
    ...
    if mes > 9:
        PERIODO = str(ano) + "-" + str(mes) + "-" + str(dia)
        print("Tipo de PERIODO: ", type(PERIODO))
        print("Mes maior que 9: ", PERIODO)
        print("Exemplo de saída correta: '2024-02-1'")
    else:
        PERIODO = str(ano) + "-0" + str(mes) + "-" + str(dia)
        print("Tipo de PERIODO: ", type(PERIODO))
        print("Mes menor que 9: ", PERIODO)
        print("Exemplo de saída correta: '2024-02-1'")
    ...
    TIPO_COLETA = tipo  # opções: "insumos", "servicos", "ambos"
    ...
    def periodo_para_nome(p, tipoArquivo=None):
        # Saída esperada: ORSE-SER-SE-202401 
        ...
        if tipoArquivo == 1:
            tipo_dados_excel = "INS"
            return f"ORSE-{tipo_dados_excel}-SE-{ano}-{mes}"
        elif tipoArquivo == 2:
            tipo_dados_excel = "SER"
            return f"ORSE-{tipo_dados_excel}-SE-{ano}-{mes}"
        else: # Se tipoArquivo for None ou outro valor
            # return f"{ano}-{mes}"
    ...
    def coletar_letra(tipo, periodo, letra, max_paginas=None):
        url_base = f"https://orse.cehop.se.gov.br/{'insumos' if tipo=='insumo' else 'servicos'}argumento.asp?tarefa=consultar"
        print("Valor de URL_BASE: ", url_base)
        ...
        for pagina in range(1, max_paginas+1 if max_paginas else 9999):
            ...
            try:                                                                                 #timeout=(10, 20)
                response = session.post(url_base + f"&page={pagina}", headers=headers, data=data, timeout=(60))
                print("VALOR DE RESPONSE (LINK): ", response)
                
                linhas = extrair_linhas(response.text)
                print("VALOR DE LINHAS: ", linhas)
    ...
    if TIPO_COLETA == "ambos":
        desktop = os.path.join(os.path.expanduser("~"), "Desktop")
        nome_excel = os.path.join(desktop, f"ORSE_{periodo_para_nome(PERIODO)}_AMBOS.xlsx")
    elif TIPO_COLETA == "servicos":
        desktop = os.path.join(os.path.expanduser("~"), "Desktop")
        nome_excel = os.path.join(desktop, f"ORSE_{periodo_para_nome(PERIODO)}_SERVICOS.xlsx")
    elif TIPO_COLETA == "insumos":
        desktop = os.path.join(os.path.expanduser("~"), "Desktop") 
        nome_excel = os.path.join(desktop, f"ORSE_{periodo_para_nome(PERIODO)}_INSUMOS.xlsx")
    else:
        desktop = os.path.join(os.path.expanduser("~"), "Desktop")
    ...
    with pd.ExcelWriter(nome_excel, engine='xlsxwriter') as writer:
        if not df_insumos.empty:
            df_insumos.to_excel(writer, sheet_name=f"{periodo_para_nome(PERIODO, tipoArquivo=1)}", index=False)
        if not df_servicos.empty:
            df_servicos.to_excel(writer, sheet_name=f"{periodo_para_nome(PERIODO, tipoArquivo=2)}", index=False)
    
    horario_termino_scraping = datetime.now().time()
    
    print("Horário de início do scraping: ", horario_inicio_scraping)
    print("Horário de término do scraping: ", horario_termino_scraping)
